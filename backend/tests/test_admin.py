...
def test_first_signup_is_promoted_to_admin(client):
    headers, _user_id = signup(client, "first@proair.com")
    me = client.get("/auth/me", headers=headers).json()
    assert me["role"] == "admin"


def test_second_signup_is_regular_user(client):
    signup(client, "first@proair.com")
    headers, _user_id = signup(client, "second@proair.com")
    me = client.get("/auth/me", headers=headers).json()
    assert me["role"] == "user"


def test_admin_routes_require_admin(client):
    signup(client, "admin@proair.com")
    user_headers, _ = signup(client, "user@proair.com")
...
def test_admin_can_list_all_users(client):
    admin_headers, _ = signup(client, "admin@proair.com")
    signup(client, "user@proair.com")
...
def test_admin_can_promote_and_demote_user(client):
    admin_headers, _ = signup(client, "admin@proair.com")
    _, user_id = signup(client, "user@proair.com")
...
def test_cannot_demote_last_remaining_admin(client):
    admin_headers, admin_id = signup(client, "admin@proair.com")
...
def test_can_demote_admin_when_another_admin_remains(client):
    admin_headers, admin_id = signup(client, "admin@proair.com")
    _, second_id = signup(client, "second@proair.com")
...
def test_admin_assigns_new_company_to_user(client):
    admin_headers, _ = signup(client, "admin@proair.com")
    user_headers, user_id = signup(client, "user@proair.com")
